...
#define BUILD_MAIN_FILE "shapes.cpp"
...
fn void build_test(char *cmd)
{
    ...
    build_cmd_append(cmd, "cc -x c "BUILD_MAIN_FILE);
    build_cmd_append(cmd, " -o ./"BUILD_DIR"/"PROJECT_NAME);   // Output
    build_cmd_append(cmd, " -lX11");                         // Libs
    build_cmd_append(cmd, " -ggdb -g3");                     // Debug
    build_cmd_append(cmd, " -Wall -Wextra");                 // Warnings
    ...
    build_cmd_append(cmd, "valgrind --leak-check=full --leak-check=full --show-leak-kinds=all ./"BUILD_DIR"/"PROJECT_NAME);
    build_cmd_finish(cmd);
}
